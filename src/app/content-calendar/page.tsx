  ...
  return (
    <div>
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <button onClick={() => openCreate()} className="rounded-lg bg-[#8B5CF6] px-4 py-2 text-sm font-medium transition-colors hover:bg-[#7C3AED]">+ New Post</button>
      </div>
        ...
        {editingPost && (
          <div className="space-y-4">
            <div>
              ...
              <input value={editingPost.title} onChange={e => setEditingPost({ ...editingPost, title: e.target.value })} className="w-full rounded-lg border border-[#222] bg-[#0a0a0a] px-3 py-2 text-sm text-white outline-none focus:border-[#8B5CF6]" />
            </div>
            <div>
              <label className="mb-1 block text-xs text-[#888]">Content</label>
              <textarea value={editingPost.content} onChange={e => setEditingPost({ ...editingPost, content: e.target.value })} rows={5} className="w-full rounded-lg border border-[#222] bg-[#0a0a0a] px-3 py-2 text-sm text-white outline-none focus:border-[#8B5CF6]" />
            </div>
            <div className="grid grid-cols-3 gap-4">
              <div>
                <label className="mb-1 block text-xs text-[#888]">Date</label>
                <input type="date" value={editingPost.scheduledDate} onChange={e => setEditingPost({ ...editingPost, scheduledDate: e.target.value })} className="w-full rounded-lg border border-[#222] bg-[#0a0a0a] px-3 py-2 text-sm text-white outline-none focus:border-[#8B5CF6]" />
              </div>
              <div>
                <label className="mb-1 block text-xs text-[#888]">Time</label>
                <input type="time" value={editingPost.scheduledTime} onChange={e => setEditingPost({ ...editingPost, scheduledTime: e.target.value })} className="w-full rounded-lg border border-[#222] bg-[#0a0a0a] px-3 py-2 text-sm text-white outline-none focus:border-[#8B5CF6]" />
              </div>
              <div>
                <label className="mb-1 block text-xs text-[#888]">Platform</label>
                <select value={editingPost.platform} onChange={e => setEditingPost({ ...editingPost, platform: e.target.value as ContentPost['platform'] })} className="w-full rounded-lg border border-[#222] bg-[#0a0a0a] px-3 py-2 text-sm text-white outline-none focus:border-[#8B5CF6]">
            ...
            <div>
              ...
              <select value={editingPost.status} onChange={e => setEditingPost({ ...editingPost, status: e.target.value as ContentPost['status'] })} className="w-full rounded-lg border border-[#222] bg-[#0a0a0a] px-3 py-2 text-sm text-white outline-none focus:border-[#8B5CF6]">
            ...
            <div className="flex justify-between pt-2">
              {posts.find(p => p.id === editingPost.id) && (
                <button onClick={() => deletePost(editingPost.id)} className="rounded-lg px-4 py-2 text-sm text-[#8B5CF6] transition-colors hover:bg-[#8B5CF6]/10">Delete</button>
              )}
              <div className="ml-auto flex gap-2">
                ...
                <button onClick={savePost} className="rounded-lg bg-[#8B5CF6] px-4 py-2 text-sm font-medium transition-colors hover:bg-[#7C3AED]">Save</button>
              </div>
            </div>
          </div>
        )}
