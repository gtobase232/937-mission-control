  ...
  return (
    <div>
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <button onClick={openCreate} className="rounded-lg bg-[#8B5CF6] px-4 py-2 text-sm font-medium transition-colors hover:bg-[#7C3AED]">+ Add Agent</button>
      </div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input value={editingMember.name} onChange={e => setEditingMember({ ...editingMember, name: e.target.value })} className="w-full rounded-lg border border-[#222] bg-[#0a0a0a] px-3 py-2 text-sm text-white outline-none focus:border-[#8B5CF6]" />
              </div>
              <div>
                <label className="mb-1 block text-xs text-[#888]">Avatar Emoji</label>
                <input value={editingMember.avatar} onChange={e => setEditingMember({ ...editingMember, avatar: e.target.value })} className="w-full rounded-lg border border-[#222] bg-[#0a0a0a] px-3 py-2 text-sm text-white outline-none focus:border-[#8B5CF6]" />
              </div>
            </div>
            <div>
              <label className="mb-1 block text-xs text-[#888]">Role</label>
              <input value={editingMember.role} onChange={e => setEditingMember({ ...editingMember, role: e.target.value })} className="w-full rounded-lg border border-[#222] bg-[#0a0a0a] px-3 py-2 text-sm text-white outline-none focus:border-[#8B5CF6]" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="mb-1 block text-xs text-[#888]">Status</label>
                <select value={editingMember.status} onChange={e => setEditingMember({ ...editingMember, status: e.target.value as TeamMember['status'] })} className="w-full rounded-lg border border-[#222] bg-[#0a0a0a] px-3 py-2 text-sm text-white outline-none focus:border-[#8B5CF6]">
            ...
            <div>
              ...
              <input value={editingMember.currentTask} onChange={e => setEditingMember({ ...editingMember, currentTask: e.target.value })} className="w-full rounded-lg border border-[#222] bg-[#0a0a0a] px-3 py-2 text-sm text-white outline-none focus:border-[#8B5CF6]" />
            </div>
            <div className="flex justify-between pt-2">
              {team.find(m => m.id === editingMember.id) && (
                <button onClick={() => deleteMember(editingMember.id)} className="rounded-lg px-4 py-2 text-sm text-[#8B5CF6] transition-colors hover:bg-[#8B5CF6]/10">Delete</button>
              )}
              <div className="ml-auto flex gap-2">
                ...
                <button onClick={saveMember} className="rounded-lg bg-[#8B5CF6] px-4 py-2 text-sm font-medium transition-colors hover:bg-[#7C3AED]">Save</button>
              </div>
            </div>
          </div>
        )}
