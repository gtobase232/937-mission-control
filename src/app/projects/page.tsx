  ...
  return (
    <div>
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <button onClick={openCreate} className="rounded-lg bg-[#8B5CF6] px-4 py-2 text-sm font-medium transition-colors hover:bg-[#7C3AED]">
          + New Project
        </button>
      </div>
          ...
          >
            ...
            <div className="mb-4">
              ...
              <div className="h-1.5 rounded-full bg-[#222]">
                <div className="h-full rounded-full bg-[#8B5CF6] transition-all" style={{ width: `${project.progress}%` }} />
              </div>
            </div>
        ...
        {editingProject && (
          <div className="space-y-4">
            <div>
              ...
              <input value={editingProject.name} onChange={e => setEditingProject({ ...editingProject, name: e.target.value })} className="w-full rounded-lg border border-[#222] bg-[#0a0a0a] px-3 py-2 text-sm text-white outline-none focus:border-[#8B5CF6]" />
            </div>
            <div>
              <label className="mb-1 block text-xs text-[#888]">Description</label>
              <textarea value={editingProject.description} onChange={e => setEditingProject({ ...editingProject, description: e.target.value })} rows={3} className="w-full rounded-lg border border-[#222] bg-[#0a0a0a] px-3 py-2 text-sm text-white outline-none focus:border-[#8B5CF6]" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="mb-1 block text-xs text-[#888]">Status</label>
                <select value={editingProject.status} onChange={e => setEditingProject({ ...editingProject, status: e.target.value as ProjectStatus })} className="w-full rounded-lg border border-[#222] bg-[#0a0a0a] px-3 py-2 text-sm text-white outline-none focus:border-[#8B5CF6]">
              ...
              <div>
                ...
                <input type="number" min="0" max="100" value={editingProject.progress} onChange={e => setEditingProject({ ...editingProject, progress: Number(e.target.value) })} className="w-full rounded-lg border border-[#222] bg-[#0a0a0a] px-3 py-2 text-sm text-white outline-none focus:border-[#8B5CF6]" />
              </div>
            </div>
            <div className="flex justify-between pt-2">
              {projects.find(p => p.id === editingProject.id) && (
                <button onClick={() => deleteProject(editingProject.id)} className="rounded-lg px-4 py-2 text-sm text-[#8B5CF6] transition-colors hover:bg-[#8B5CF6]/10">Delete</button>
              )}
              <div className="ml-auto flex gap-2">
                ...
                <button onClick={saveProject} className="rounded-lg bg-[#8B5CF6] px-4 py-2 text-sm font-medium transition-colors hover:bg-[#7C3AED]">Save</button>
              </div>
            </div>
          </div>
        )}
